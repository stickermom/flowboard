...
export default function HomePage() {
  ...
  const addToCart = async (product: Product) => {
    if (!userId || userId.trim() === '') {
      console.error('Cannot add to cart: userId is not set');
      return;
    }
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : 'Failed to add item to cart. Please try again.');
    }
  };

  const updateQuantity = async (productId: string, newQuantity: number) => {
    if (!userId || userId.trim() === '') {
      console.error('Cannot update quantity: userId is not set');
      return;
    }
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : 'Failed to update quantity. Please try again.');
    }
  };

  const removeItem = async (productId: string) => {
    if (!userId || userId.trim() === '') {
      console.error('Cannot remove item: userId is not set');
      return;
    }
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : 'Failed to remove item. Please try again.');
    }
  };

  const addToFavorites = async (product: Product) => {
    if (!userId || userId.trim() === '') {
      console.error('Cannot add to favorites: userId is not set');
      return;
    }
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : 'Failed to update favorites. Please try again.');
    }
  };

  const removeFavorite = async (productId: string) => {
    if (!userId || userId.trim() === '') {
      console.error('Cannot remove favorite: userId is not set');
      return;
    }
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : 'Failed to remove favorite. Please try again.');
    }
  };
  ...
  // Initialize userId on client side only (fixes hydration)
  useEffect(() => {
    ...
    const loadUserData = async () => {
      try {
        const [cartData, favoritesData] = await Promise.all([
          cartService.getCartItems(userId).catch(() => []),
          favoritesService.getFavorites(userId).catch(() => []), // Handle favorites error gracefully
        ]);
        ...
        <div className="bg-white/60 backdrop-blur-xl border border-white/20 rounded-b-2xl shadow-sm px-6 py-3 pb-4 max-w-xl">
          ...
          >
            ...
            >
              ...
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-green-500 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1.5 shadow-lg">
